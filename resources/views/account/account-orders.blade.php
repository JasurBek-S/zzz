<x-app title="{{ __('app.profile_orders_title') }}">
    <!-- Order Details Modal-->
            ...
            <div class="order-lg-2 mb-3 mb-lg-0 pt-lg-2">
                <nav aria-label="breadcrumb">
                    <ol
                        ...
                        <li class="breadcrumb-item text-nowrap active" aria-current="page">{{ __('app.profile_orders_title') }}</li>
                    </ol>
                </nav>
            </div>
            <div class="order-lg-1 pr-lg-4 text-center text-lg-left">
                <h1 class="h3 text-light mb-0">{{ __('app.profile_orders_title') }}</h1>
            </div>
        </div>
    </div>
    <!-- Page Content-->
            ...
            <section class="col-lg-8">
                ...
                <div class="table-responsive font-size-md">
                    <table class="table table-hover mb-0">
                        <thead>
                            <tr>
                                <th>{{ __('app.profile_orders_table_order') }}</th>
                                <th>{{ __('app.profile_orders_table_date') }}</th>
                                <th>{{ __('app.profile_orders_table_status') }}</th>
                                <th>{{ __('app.profile_orders_table_total') }}</th>
                                <th>{{ __('app.profile_orders_table_action') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                            function order_status($id, $stats){
                            $language = app()->getLocale();
                            foreach($stats as $status_data){
                            if($status_data->id == $id){
                            $o_name = json_decode($status_data->name, true);
                            return $o_name[$language];
                            }
                            }
                            }
                            @endphp
                            ...
                            <tr>
                                <td class="py-3">
                                    <a class="nav-link-style font-weight-medium font-size-sm" href="#order-details"
                                        ...
                                        data-toggle="modal">{{  $order->order_number }}</a>
                                </td>
                                <td class="py-3">{{  $order->created_at }}</td>
                                <td class="py-3"><span
                                        class="badge badge-info m-0">{{  order_status($order->status, $status_order) }}</span>
                                </td>
                                <td class="py-3">{{  $order->order_number }}</td>
                                <td class="py-3"><a href="{{ route('tracking.show', [$order->order_number]) }}"
                                        data-toggle="tooltip" title="{{ __('app.profile_orders_table_tracking') }}"><i class="czi-lable"></i></a></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <hr class="pb-4">
                <!-- Pagination-->
                {!! $orders->links('vendor.pagination.account') !!}
            </section>
